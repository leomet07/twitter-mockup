...
signupForm.addEventListener("submit", (e) => {
	...
	auth.createUserWithEmailAndPassword(email, password).then((cred) => {
		...
		firebase.database().ref().child("users").child(global_user.uid).set({
			user_name: username,
			follow: { followers: { amount: { other_users_follower_amount: "0" } }, following: { amount: { other_users_follower_amount: "0" } } }
		});
...
auth.onAuthStateChanged((user) => {
	if (user) {
		...
		firebase.database().ref().child("usernames").child(global_user.uid).set(global_user.email);
	} else {
		//making verif not seen
		document.getElementById("verify").innerHTML = null;
